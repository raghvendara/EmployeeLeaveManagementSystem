...
public class MakeRequiredQuery {
	...
	public String queryForHR(String leaveType) {
		...
		return "UPDATE raghava_empleaveportal_leaves_info AS t1,raghava_empleaveportal_leave_request_info as t3 INNER JOIN "
				...
				+ "WHERE t1.emp_id = ? and t3.emp_id=? and t3.leave_id=?;";
	...
	public String queryForTL() {
		return "update raghava_empleaveportal_leave_request_info set team_lead_status='approved' where emp_id=? and leave_id=?;";
	}

	public String queryForPM() {
		return "update raghava_empleaveportal_leave_request_info set proj_man_status='approved' where emp_id=? and leave_id=?;";
	}

	public String queryForRejectTL() {
		return "update raghava_empleaveportal_leave_request_info set team_lead_status='reject' where emp_id=? and leave_id=?;";
	}

	public String queryForRejectPM() {
		return "update raghava_empleaveportal_leave_request_info set proj_man_status='reject' where emp_id=? and leave_id=?;";
	}

	public String queryForRejectHR() {
		return "update raghava_empleaveportal_leave_request_info set hr_status='reject' where emp_id=? and leave_id=?;";
	}
