...
public class UpdateDB {
	...
	public void updateDB(Date ourJavaDateObject) {
		...
		try {
			...
			while(rs.next())
			{
				...
				int compareResult = ourJavaDateObject.compareTo(rs.getDate(3));
				if(compareResult==0)
			...
			if(empLeaveList.size()!=0 && flagToCheckResultSet){
				updateLeaveStatus(connection, empLeaveList);
			}
	...
	private void updateLeaveStatus(Connection connection, List<LeaveInfoToUpdateStatusVo> empLeaveList) {
		System.out.println("@@@@@@@@@@ ****** in the updateLeaveStatus ******* @@@@@@@@@@");
		...
		while(itr.hasNext())
		{
			...
			try {
				...
				if(c>=1){
				System.out.println("@@@@@@@@@@ ****** updated the leave status in database ******* @@@@@@@@@@");
				connection.commit();
				}
			} catch (SQLException e) {
				e.printStackTrace();
